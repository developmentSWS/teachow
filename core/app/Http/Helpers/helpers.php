...
function loadCustomCaptcha($textColor, $height = 50, $width = '100%', $bgcolor = '#003')
{
    $textColor = '#' . $textColor;
    ...
    $ret .= '<input type="hidden" class="form--control form-control" name="captcha_secret" value="' . $captchaSecret . '">';
    return $ret;
}
...
function getAdvertisement($size)
{
    ...
    if($ad){
        if($ad->type == 'image'){
            $advertise = '<div class="__add text-center" data-id="'.$ad->id.'"><a href="'.$ad->redirect_url.'" target="_blank"> <img src="'.getImage(imagePath()['advertisement']['path'] . '/' . @$ad->value, @$ad->size).'" alt="Ads"></a></div>';
        }else{
            $advertise = '<div class="__add" data-id="'.$ad->id.'">'.$ad->script.'</div>';
        }
